...
def my_loss(output,target,nlm=2,wtlim=None):  
    ...
    if nlm == 1:    # norm1, NCHW times of mae
        # loss = torch.norm(output-target,1) # torch.norm is deprecated 20240913
        loss = (torch.sum((output - target)))
    elif nlm == 2:      # norm2 (NCHW)**(1/2) times of rmse
        # loss = torch.norm(output-target,2) # torch.norm is deprecated
        loss = (torch.sum((output - target)**2))**(1/2)
    elif nlm == 3:      # norm3
        # loss = torch.norm(output-target,3) # torch.norm is deprecated
        loss = (torch.sum((output - target)**3))**(1/3)
    elif nlm == 4:      # mae
...
class myDataset(Dataset):
    def __init__(self, files_lr,files_hr,indt_lr,indt_hr,hr_shape,up_factor=4,
        ...
        if self.mode == "train":
            ...
            # self.indt_hr = [indt_hr[i] for i in ind_train] # should be used
            # self.indt_lr = [indt_lr[i] for i in ind_train]
        elif self.mode == "valid":
            self.files_hr = [files_hr[i] for i in ind_valid] #
            self.files_lr = [files_lr[i] for i in ind_valid]
            # self.indt_hr = [indt_hr[i] for i in ind_valid]
            # self.indt_lr = [indt_lr[i] for i in ind_valid]
        elif self.mode == "test":
            self.files_hr = [files_hr[i] for i in ind_test] # getitem does not change self
            self.files_lr = [files_lr[i] for i in ind_test]
            # self.indt_hr = [indt_hr[i] for i in ind_test]
            # self.indt_lr = [indt_lr[i] for i in ind_test]
