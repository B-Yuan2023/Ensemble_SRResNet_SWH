...
if __name__ == '__main__':
    ...
    tlim = [datetime(2018,1,1),datetime(2018,12,31)]
    dt = 3
    ...
    # tlim = [datetime(2019,1,1),datetime(2019,12,31)]
    # dt = 3
    # files_hr, indt_hr = make_list_file_t(dir_hr,tlim,dt,tshift=0,t0_h=0,dt_h=1,ntpf=24)
    # files_lr, indt_lr = make_list_file_t(dir_lr,tlim,dt,tshift=0,t0_h=0,dt_h=1,ntpf=24)
    # Nt = len(files_lr)
    ...
    # # for a different year
    # tlim = [datetime(2020,1,1),datetime(2020,12,31)]
    # dt = 3
    # files_hr, indt_hr = make_list_file_t(dir_hr,tlim,dt,tshift=0,t0_h=0,dt_h=1,ntpf=24)
    # files_lr, indt_lr = make_list_file_t(dir_lr,tlim,dt,tshift=0,t0_h=0,dt_h=1,ntpf=24)
    # Nt = len(files_lr)
    ...
    # for a different year
    tlim = [datetime(2021,1,1),datetime(2021,12,31)]
    dt = 3
    files_hr, indt_hr = make_list_file_t(dir_hr,tlim,dt,tshift=0,t0_h=0,dt_h=1,ntpf=24)
    files_lr, indt_lr = make_list_file_t(dir_lr,tlim,dt,tshift=0,t0_h=0,dt_h=1,ntpf=24)
    Nt = len(files_lr)
    ...
    # get all hr data. Note: only for a short period, otherwise could be too large
    hr_all_train4 = np.zeros((Nt,nchl_o,len(lat),len(lon))) 
    for i in range(Nt):
        for ichl in range(nchl_o): 
            nc_f = files_hr[i][ichl]
            indt = indt_hr[i][ichl]
            ...
            mask =  nc_load_vars(nc_f,var_hr[0],[indt],lats=ll_hr[0],lons=ll_hr[1])[4]
            dat_hr[mask] = np.nan
            hr_all_train4[i,ichl,:,:] = dat_hr
            
            
    # for a different year
    tlim = [datetime(2023,1,1),datetime(2023,12,31)]
    dt = 3
    files_hr, indt_hr = make_list_file_t(dir_hr,tlim,dt,tshift=0,t0_h=0,dt_h=1,ntpf=24)
    files_lr, indt_lr = make_list_file_t(dir_lr,tlim,dt,tshift=0,t0_h=0,dt_h=1,ntpf=24)
    Nt = len(files_lr)
    ...
    # get all hr data. Note: only for a short period, otherwise could be too large
    hr_all_train5 = np.zeros((Nt,nchl_o,len(lat),len(lon))) 
    for i in range(Nt):
        for ichl in range(nchl_o): 
            nc_f = files_hr[i][ichl]
            indt = indt_hr[i][ichl]
...
# plot the distribution of input data together 
    ...
    #     var4 = hr_all_train4[:,k,:,:].flatten()
    #     var5 = hr_all_train5[:,k,:,:].flatten()
    #     var = [var0[~np.isnan(var0)],var1[~np.isnan(var1)],var2[~np.isnan(var2)],
    #            var3[~np.isnan(var3)],var4[~np.isnan(var4)],var5[~np.isnan(var5)],
    #            ]
    #     leg = ['2018','2019', '2020','2021', '2022','2023'] 
    #     figname = opath_st+"dist_c%d" % (ichl)+'_years%d'%(len(var))+'.png'
    ...
    # get all hr data. Note: only for a short period, otherwise could be too large
    hr_all_train6 = np.zeros((Nt,nchl_o,len(lat),len(lon))) 
    for i in range(Nt):
        for ichl in range(nchl_o): 
            nc_f = files_hr[i][ichl]
            indt = indt_hr[i][ichl]
    ...
    for k in range(nchl_o): # nchl_o, save for all time steps. 
        ...
        var4 = hr_all_train4[:,k,:,:].flatten()
        var5 = hr_all_train5[:,k,:,:].flatten()
        var6 = hr_all_train6[:,k,:,:].flatten()
        var = [var6[~np.isnan(var6)],
               var3[~np.isnan(var3)],var4[~np.isnan(var4)],var5[~np.isnan(var5)],
               ]
        leg = ['2018-2020','2021', '2022','2023'] 
        figname = opath_st+"dist_c%d" % (ichl)+'_years%d'%(len(var))+'.png'
        plot_distri(var,figname,nbin=nbin,lim=lim,axlab=axlab,leg=leg,
                       figsize=(7.4, 2.5), fontsize=12,capt='',
                       xlim=xlim,ylim=ylim)                